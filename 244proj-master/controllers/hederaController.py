...
class HederaController(object):
  ...
  def _handle_ConnectionUp (self, event):
    ...
    self.log.info("Saw switch come up: %s", sw_str)
    ...
    if name_str not in self.t.switches():
      ...
      return
    if sw is None:
      self.log.info("Added fresh switch %s" % sw_str)
      sw = Switch()
      self.switches[event.dpid] = sw
      sw.connect(event.connection)
    else:
      self.log.info("Odd - already saw switch %s come up" % sw_str)
      sw.connect(event.connection)
    sw.connection.send(of.ofp_set_config(miss_send_len=MISS_SEND_LEN))

    if len(self.switches) == len(self.t.switches()):
      self.log.info("Woo!  All switches up")
